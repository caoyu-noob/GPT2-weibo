...
def setup_train_args():
    ...
    parser.add_argument('--train_tokenized_path', default='data/train_tokenized.bin', type=str,
                        required=False,
    ...
    parser.add_argument('--batch_size', default=256, type=int, required=False, help='训练batch size')
    parser.add_argument('--lr', default=1e-4, type=float, required=False, help='学习率')
    parser.add_argument('--warmup_steps', default=2000, type=int, required=False, help='warm up步数')
    parser.add_argument('--log_step', default=5000, type=int, required=False, help='多少步汇报一次loss')
    parser.add_argument('--gradient_accumulation', default=8, type=int, required=False, help='梯度积累')
    ...
    parser.add_argument('--train_mmi_tokenized_path', default='data/train_mmi_tokenized.bin', type=str,
                        required=False,
    ...
    parser.add_argument("--local_rank", type=int, default=-1, help="当前GPU排序 (-1: not distributed)")
    parser.add_argument("--tensor_cache", action='store_true', help="使用的是否为基于torch tensor的cache")
    parser.add_argument("--max_len", default=180, type=int, help="最大的输入长度，超过此长度则会被截断")
...
def preprocess_raw_data(args, tokenizer, tokenized_path, n_ctx, mode='normal'):
    """
    ...
    :return:
    """
    logger.info("tokenizing raw data,raw data path:{}, token output path:{}".format(args.train_path,
        ...
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            if "\t" in dialogue:
                ...
                for utterance in utterances:
                    ...
                    dialogue_ids.extend(tokenizer.encode(utterance)[1:-1])
                ...
                if args.tensor_cache:
                    processed_data.append(dialogue_ids + [0] * (n_ctx - len(dialogue_ids)))
                else:
        ...
        for dialogue_index, dialogue in enumerate(tqdm(train_data)):
            if "\t" in dialogue:
                ...
                if args.tensor_cache:
                    ...
                    processed_data.append(dialogue_ids)
    if args.tensor_cache:
        processed_data = torch.LongTensor(processed_data)
        torch.save(processed_data, tokenized_path)
    else:
...
def collate_fn(batch):
    ...
    if isinstance(batch[0], list):
        ...
        for btc_idx in range(btc_size):
            length = sum(batch[btc_idx].gt(0)).item()
            if max_input_len < length:
                max_input_len = length
        for btc_idx in range(btc_size):
            batch[btc_idx] = batch[btc_idx][: max_input_len].unsqueeze(0)
        return torch.cat(batch, dim=0)

def load_data(args, tokenizer, n_ctx):
    if args.train_mmi:
        if not os.path.exists(args.train_mmi_tokenized_path):
            preprocess_raw_data(args, tokenizer, args.train_mmi_tokenized_path, n_ctx, 'mmi')
        if args.tensor_cahce:
            data = torch.load(args.train_mmi_tokenized_path)
        else:
            with open(args.train_mmi_tokenized_path, 'rb') as f:
                data = pickle.load(f)
    else:
        if not os.path.exists(args.train_tokenized_path):
            preprocess_raw_data(args, tokenizer, args.train_tokenized_path, n_ctx)
        if args.tensor_cache:
            data = torch.load(args.train_tokenized_path)
        else:
            with open(args.train_tokenized_path, 'rb') as f:
                data = pickle.load(f)
    return data
...
def train(model, device, train_list, multi_gpu, args, logdir):
    ...
    if args.distributed:
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=loader_batch_size, shuffle=False,
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=loader_batch_size, shuffle=True,
    ...
    base_optimizer = transformers.AdamW(model.parameters(), lr=args.lr, correct_bias=True)
    ...
    optimizer = NoamOpt(768, args.warmup_steps, base_optimizer, lr=args.lr, linear_schedule=True,
                        total_steps=total_steps, apex_level=None, loss_weight=None,
                        extra_module_lr_rate=1)
    ...
    for epoch in tqdm(range(args.epochs), desc='Epoch'):
        ...
        for batch_idx, input_ids in enumerate(tqdm_data):
            ...
            rest_size = 0 if device == 'cpu' else (torch.cuda.device_count() - input_ids.size(0) % torch.cuda.device_count())
            if rest_size != 0:
                input_ids = torch.cat([input_ids] + [input_ids[:1, :]] * rest_size, dim=0)
            try:
                ...
                if (batch_idx + 1) % args.gradient_accumulation == 0:
                    ...
                    # 进行warm up
                    overall_step += 1
                    log_loss = (log_loss * batch_idx + loss.item()) / (batch_idx + 1)
                    log_acc = (log_acc * batch_idx + accuracy.item()) / (batch_idx + 1)
                    tqdm_data.set_postfix({'loss': log_loss, 'acc': log_acc})
                    # 更新日志与tnesorboardX信息
                    if (overall_step + 1) % args.log_step == 0:
                        if args.local_rank in [-1, 0]:
                            logger.info('loss is %.5f, acc is %.5f', log_loss, log_acc)
...
def evaluate(model, device, test_list, multi_gpu, args):
    ...
    with torch.no_grad():
        for batch_idx, input_ids in enumerate(test_dataloader):
            input_ids.to(device)
            rest_size = input_ids.size(0) % torch.cuda.device_count()
            if rest_size != 0:
                input_ids = torch.cat([input_ids] + [input_ids[:1, :]] * rest_size, dim=0)
            outputs = model.forward(input_ids=input_ids)
            loss, accuracy = calculate_loss_and_accuracy(outputs, labels=input_ids, device=device)
...
def main():
    ...
    if args.local_rank in [-1, 0]:
        ...
        if args.train_mmi:
            logdir = os.path.join('runs', current_time + '_MMI')
        else:
    ...
    data_list = load_data(args, tokenizer, args.max_len)
    train_list, test_list = train_test_split(data_list, test_size=0.0005, random_state=1)
    ...
    # 开始训练
    train(model, device, train_list, multi_gpu, args, logdir)
    # 测试模型
